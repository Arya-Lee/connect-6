...
--对字符串编解码 以达到可以同时落多子并显示出来
prints x = putStrLn $ strdos x                                                        --打印单个颜色的棋子
printchess a b =  prints $ concat [a,b]                                                     -- 打印两种颜色的棋子
------------------------------------------------------------------------
qsort :: Ord a => [a] -> [a]     
qsort []=[]
...
get_part_before_right_braket s = if head s==')' then ""
                                     else head s: (get_part_before_right_braket $tail s)--取s右括号之前内容的函数
get_part_after_left_braket s = if head s=='(' then tail s 
                                   else get_part_after_left_braket $tail s  --取s左括号之后内容的函数
xb1 :: [(a, b)] -> a
xb1 l=fst $ l!!0
--取输出落子的第一个序对中的第一个值，即黑方落子的第一个坐标的横坐标
xb2 :: [(a, b)] -> a
xb2 l=fst $ l!!1
--取输出落子的第二个序对中的第一个值，即黑方落子的第二个坐标的横坐标
yb1 :: [(a, b)] -> b
yb1 l=snd $ l!!0
--取输出落子的第一个序对中的第二个值，即黑方落子的第一个坐标的纵坐标
yb2 :: [(a, b)] -> b
yb2 l=snd $ l!!1
--取输出落子的第二个序对中的第一个值，即黑方落子的第一个坐标的纵坐标
transfertotwobits x
   |x>10||x==10 =show x
   |x<10 ="0"++ show x  
...
ai_black=do
  ...
  let aab = lines mdatablack     
      ...
      everyonein [] ta = True
      everyonein (s:st) ta = s `elem` ta && everyonein st ta   --判断a是否在b中
      positions :: Eq a => a -> [a] -> [Int]
      positions x xs = [pos | (x',pos) <- zip xs [1..length xs],x' == x] --找出等于xs 中x的位置
      m :: Int -> String
      p :: Int -> String
      m n = aab !! n      --取aab数据集（black.csv）的第n行（n从0开始）
      p q = aaw !! q      --取aaw数据集（white.csv）的第n行（n从0开始）
      ...
      tbs :: Int -> Int
      tb :: Int -> Int
      tbh n = read $ take 2 $ m n :: Int     --black.csv文件中第n行对应时间里的小时
      tbmin n = read $ take 2 $ (tails $ m n) !! 3 :: Int   --black.csv文件中第n行对应时间里的分钟
      tbs n = read $ take 2 $ (tails $ m n) !! 6 :: Int   --black.csv文件中第n行对应时间里的秒
      tb n = 3600 * tbh n + 60 * tbmin n + tbs n   --黑方对应时刻的总秒数，利用该变量去判断时间上的违规问题
      
      twh :: Int -> Int
      twmin :: Int -> Int
      tws :: Int -> Int
      tw :: Int -> Int
      twh q = read $ take 2 $ p q :: Int      --white.csv文件中第n行对应时间里的小时
      twmin q = read $ take 2 $ (tails $ p q) !! 3 :: Int   --white.csv文件中第n行对应时间里的分钟
      tws q = read $ take 2 $ (tails $ p q) !! 6 :: Int    --white.csv文件中第n行对应时间里的秒
      tw q = 3600 * twh q + 60 * twmin q + tws q   --白方对应时刻的总秒数
      ...
      chess_coms (x:xs) b = chess_coms xs (chess_com x b) --把带符号位的黑棋或者白棋放入带符号位的空棋盘中 不能重复
      
      chess_comr (x,y,z) b = map (com_bwer (x,y,z)) b
      
      chess_comrs [x] b = chess_comr x b
      chess_comrs (x:xs) b = chess_comrs xs (chess_comr x b)--用空子位置取代黑棋或白棋的位置 不能重复
      
      chess_comt (x,y,z) b = map (com_bwet (x,y,z)) b
      
      chess_comts [x] b = chess_comt x b
      chess_comts (x:xs) b = chess_comts xs (chess_comt x b)--用空子位置取代黑棋或白棋的位置 可以重复
      ...
      ext_list [x] = ext x
      ext_list (x:xs) = ext x ++ ext_list xs--提取带符号位的棋盘空子
      
      shan x y = if x == y then [] else [y]
      shans a [b]= shan a b
      shans a (x:xs) = shan a x ++ shans a xs --把列表中等于b的部分删除
 ...
 --    winss s = length $ filter (>=1) $ wins s
      ...
      ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
      ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------       
      bbb = if nj_ookk 0 1 5 >= 1 then ttt 0 1 5                                                             --xia
       else if nj_ookk 0 1 4 >= 1 then ttt 0 1 4                                                             --xia    
       else if nj_ookk 0 2 5 >= 1 then ttt 0 2 5                                                             --xia
       else if nj_ookk 0 2 4 >= 1 then ttt 0 2 4                                                             --xia    
       else if nj_ookk 1 1 5 >= 1 then ttt 1 1 5                                                             -- w huo 5  du
       else if nj_ookk 1 1 4 >= 1 then ttt 1 1 4                                                             -- w huo 4  du  
      -- w mian 5 mian 5 
       else if nj_ookk 1 2 5 >= 1 && nj_ookk 1 2 4 >= 1 then (take 1 $ ttt 1 2 5) ++ (take 1 $ ttt 1 2 4)    --w mian 5 && w mian 4
       else if nj_ookk 1 2 5 >= 1 && nj_ookk 0 1 3 >= 1 then (take 1 $ ttt 1 2 5) ++ (take 1 $ ttt 0 1 3)    --w mian 5 && b huo 3
       else if nj_ookk 1 2 5 >= 1 && nj_ookk 0 3 3 >= 1 then (take 1 $ ttt 1 2 5) ++ (take 1 $ ttt 0 3 3)    --                  menglong 3
       else if nj_ookk 1 2 5 >= 1 && nj_ookk 0 2 3 >= 1 then (take 1 $ ttt 1 2 5) ++ (take 1 $ ttt 0 2 3)    --                  mian 3 
       else if nj_ookk 1 2 5 >= 1 && nj_ookk 0 1 2 >= 1 then (take 1 $ ttt 1 2 5) ++ (take 1 $ ttt 0 1 2)    --                  huo 2
       else if nj_ookk 1 2 5 >= 1 && nj_ookk 0 2 2 >= 1 then (take 1 $ ttt 1 2 5) ++ (take 1 $ ttt 0 2 2)    --                  mian 2  
       else if nj_ookk 1 2 5 >= 1 && nj_ookk 1 1 3 >= 1 then (take 1 $ ttt 1 2 5) ++ (take 1 $ ttt 1 1 3)    --w mian 5 && w huo 3
       else if nj_ookk 1 2 5 >= 1 && nj_ookk 1 3 3 >= 1 then (take 1 $ ttt 1 2 5) ++ (take 1 $ ttt 1 3 3)    --                  menglong 3
       else if nj_ookk 1 2 5 >= 1 && nj_ookk 1 2 3 >= 1 then (take 1 $ ttt 1 2 5) ++ (take 1 $ ttt 1 2 3)    --                  mian 3
       else if nj_ookk 1 2 5 >= 1 && nj_ookk 1 1 2 >= 1 then (take 1 $ ttt 1 2 5) ++ (take 1 $ ttt 1 1 2)    --                  huo 2
       else if nj_ookk 1 2 5 >= 1 && nj_ookk 1 2 2 >= 1 then (take 1 $ ttt 1 2 5) ++ (take 1 $ ttt 1 2 2)    --                  mian 2
       else if nj_ookk 1 2 5 >= 1 && nj_ookk 0 1 1 >= 1 then (take 1 $ ttt 1 2 5) ++ (take 1 $ ttt 0 1 1)    --w mian 5 && b huo 1     
       else if nj_ookk 1 2 5 >= 1 && nj_ookk 0 2 1 >= 1 then (take 1 $ ttt 1 2 5) ++ (take 1 $ ttt 0 2 1)    --                  mian 1
       else if nj_ookk 1 2 5 >= 1 && nj_ookk 1 1 1 >= 1 then (take 1 $ ttt 1 2 5) ++ (take 1 $ ttt 1 1 1)    --w mian 5 && w huo 1
       else if nj_ookk 1 2 5 >= 1 && nj_ookk 1 2 1 >= 1 then (take 1 $ ttt 1 2 5) ++ (take 1 $ ttt 1 2 1)    --                  mian 1
      --w mian 4 mian 4 
       else if nj_ookk 1 2 4 >= 1 && nj_ookk 0 1 3 >= 1 then (take 1 $ ttt 1 2 4) ++ (take 1 $ ttt 0 1 3)    --w mian 4 && b huo 3
       else if nj_ookk 1 2 4 >= 1 && nj_ookk 0 3 3 >= 1 then (take 1 $ ttt 1 2 5) ++ (take 1 $ ttt 0 3 3)    --                  menglong 3 
       else if nj_ookk 1 2 4 >= 1 && nj_ookk 0 2 3 >= 1 then (take 1 $ ttt 1 2 4) ++ (take 1 $ ttt 0 2 3)    --                  mian 3 
       else if nj_ookk 1 2 4 >= 1 && nj_ookk 0 1 2 >= 1 then (take 1 $ ttt 1 2 4) ++ (take 1 $ ttt 0 1 2)    --                  huo 2
       else if nj_ookk 1 2 4 >= 1 && nj_ookk 0 2 2 >= 1 then (take 1 $ ttt 1 2 4) ++ (take 1 $ ttt 0 2 2)    --                  mian 2  
       else if nj_ookk 1 2 4 >= 1 && nj_ookk 1 1 3 >= 1 then (take 1 $ ttt 1 2 4) ++ (take 1 $ ttt 1 1 3)    --w mian 4 && w huo 3
       else if nj_ookk 1 2 4 >= 1 && nj_ookk 1 3 3 >= 1 then (take 1 $ ttt 1 2 5) ++ (take 1 $ ttt 1 3 3)    --                  menglong 3 
       else if nj_ookk 1 2 4 >= 1 && nj_ookk 1 2 3 >= 1 then (take 1 $ ttt 1 2 4) ++ (take 1 $ ttt 1 2 3)    --                  mian 3
       else if nj_ookk 1 2 4 >= 1 && nj_ookk 1 1 2 >= 1 then (take 1 $ ttt 1 2 4) ++ (take 1 $ ttt 1 1 2)    --                  huo 2
       else if nj_ookk 1 2 4 >= 1 && nj_ookk 1 2 2 >= 1 then (take 1 $ ttt 1 2 4) ++ (take 1 $ ttt 1 2 2)    --                  mian 2
       else if nj_ookk 1 2 4 >= 1 && nj_ookk 0 1 1 >= 1 then (take 1 $ ttt 1 2 4) ++ (take 1 $ ttt 0 1 1)    --w mian 4 && b huo 1     
       else if nj_ookk 1 2 4 >= 1 && nj_ookk 0 2 1 >= 1 then (take 1 $ ttt 1 2 4) ++ (take 1 $ ttt 0 2 1)    --                  mian 1
       else if nj_ookk 1 2 4 >= 1 && nj_ookk 1 1 1 >= 1 then (take 1 $ ttt 1 2 4) ++ (take 1 $ ttt 1 1 1)    --w mian 4 && w huo 1
       else if nj_ookk 1 2 4 >= 1 && nj_ookk 1 2 1 >= 1 then (take 1 $ ttt 1 2 4) ++ (take 1 $ ttt 1 2 1)    --                  mian 1
      --b huo 3 huo 3 
       ...
       else if nj_ookk 0 1 3 >= 1 && nj_ookk 1 1 3 >= 1 then (take 1 $ ttt 0 1 3) ++ (take 1 $ ttt 1 1 3)    --                 w huo 3 
       else if nj_ookk 0 1 3 >= 1 && nj_ookk 1 2 3 >= 1 then (take 1 $ ttt 0 1 3) ++ (take 1 $ ttt 1 2 3)    --                     mian 3
      --w huo 3 huo 3 
      ...
      --b mian 3 mian 3
       else if nj_ookk 1 3 3 >= 1 && nj_ookk 0 1 2 >= 1 then (take 1 $ ttt 1 3 3) ++ (take 1 $ ttt 0 1 2)    --w menglong 3 && b huo 2 
       else if nj_ookk 1 3 3 >= 1 && nj_ookk 0 2 3 >= 1 then (take 1 $ ttt 1 3 3) ++ (take 1 $ ttt 0 2 3)    --w menglong 3 && b mian 3
       else if nj_ookk 1 3 3 >= 1 && nj_ookk 0 1 1 >= 1 then (take 1 $ ttt 1 3 3) ++ (take 1 $ ttt 0 1 1)    --w menglong 3 && b huo 1
       else if nj_ookk 1 3 3 >= 1 && nj_ookk 0 2 2 >= 1 then (take 1 $ ttt 1 3 3) ++ (take 1 $ ttt 0 2 2)    --w menglong 3 && b mian 2 
       
       else if nj_ookk 1 1 2 >= 1 && nj_ookk 0 2 3 >= 1 then (take 1 $ ttt 1 1 2) ++ (take 1 $ ttt 0 2 3)    --w huo 2 && b mian 3
       else if nj_ookk 1 1 2 >= 1 && nj_ookk 0 1 1 >= 1 then (take 1 $ ttt 1 1 2) ++ (take 1 $ ttt 0 1 1)    --w huo 2 && b huo 1
       else if nj_ookk 1 1 2 >= 1 && nj_ookk 0 2 2 >= 1 then (take 1 $ ttt 1 1 2) ++ (take 1 $ ttt 0 2 2)    --w huo 2 && b mian 2
      --w mian 3 mian 3 du liang ge
       else if nj_ookk 1 3 3 >= 1 then ttt 1 3 3                                                             --w menglong 3  
       else if nj_ookk 1 1 2 >= 1 then ttt 1 1 2                                                             --w huo 2 
       
       else if nj_ookk 0 1 1 >= 1 then ttt 0 1 1                                                             --b huo 1 
       else if nj_ookk 1 1 1 >= 1 && nj_ookk 0 2 1 >= 1 then ttt 1 1 1 ++ ttt 0 2 1                          --w huo 1 && b mian 1
      ...
      --黑棋判断与落子
      bbb1 = if nj_ookk 0 1 5 >= 1 then take 1 $ ext_list redefine_list_com else []
      
      bbb_out = if length bbb == 1 then  bbb ++ bbb1   -- zhi xia 1 zi shi jia yi zi 
           else bbb
     --黑棋最后输出 ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
      
      pairs_of_black 1=(xb1 bbb_out,yb1 bbb_out)
	  --黑棋输出的第一个子
      pairs_of_black 2=(xb2 bbb_out,yb2 bbb_out)
	  --黑棋输出的第二个子
      ...
      m_black=timeNow++","++(show $ lb+1)++",\""++(show $ pairs_of_black 1)++"\",\""++(show $ pairs_of_black 2)++"\"\n"
  when (length m_black > 0) $ do
        appendFile "black.csv" m_black--将输出结果存入black.csv文件中 
  printchess bla whi  --写入数据后在终端上打印棋盘                
  putStrLn "black :"
  print bbb_out    --显示黑方落子位置
